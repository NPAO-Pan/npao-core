...
export default class HypeTrack {
  ...
  static _onReady() {
    if (game.npao.hypeTrack) {
      game.npao.hypeTrack._checkForHypeTracksPlaylist();
      game.npao.playHype = game.npao.hypeTrack.playHype.bind(
        game.npao.hypeTrack
      );
    }
  }
  ...
  async _checkForHypeTracksPlaylist() {
    ...
    const hypePlaylist = game.playlists.getName(
      config.HYPETRACK_CONFIG.HypeTrack.playlistName
    );
  ...
  async _createHypeTracksPlaylist() {
    return await Playlist.create({
      name: config.HYPETRACK_CONFIG.HypeTrack.playlistName,
    });
  }
  ...
  async _processHype(combat, update) {
    ...
    const pauseOthers = game.settings.get(
      config.moduleName,
      config.SETTINGS_KEYS.HypeTrack.pauseOthers
    );
