...
//   else return game.settings.get("monks-chat-timer", key);
// };

// export class MonksChatTimer {
//   static init() {
//     if (game.MonksChatTimer == undefined) game.MonksChatTimer = MonksChatTimer;
...
//     let calcTime =
//       (Math.abs(timePart[0]) +
//         (timePart.length > 1 ? Math.abs(timePart[1]) * 60 : 0) +
//         (timePart.length > 2 ? Math.abs(timePart[2]) * 3600 : 0)) *
//       1000 *
//       (time.startsWith("-") ? -1 : 1);

//     let frmtTime = new Date(calcTime < 0 ? 0 : calcTime)
//       .toISOString()
//       .substr(11, 8);
//     let content = `<div class="timer-msg"><div class="timer-flavor">${
//       options.flavor
//     }</div><div class="timer-time">${frmtTime}</div><div class="timer-bar"><div></div></div><div class="complete-msg">${i18n(
//       "MonksChatTimer.Complete"
//     )}</div></div>`;
...
//     if (options.whisper) messageData.whisper = options.whisper;
...
//             (timePart.length > 2 ? Math.abs(timePart[2]) * 3600 : 0)) *
//           1000 *
//           (found[0].startsWith("-") ? -1 : 1);

//         let flavor = null;
//         let followup = null;
//         if (found.length > 1) flavor = found[1].trim();
...
//           },
//         };
//         let frmtTime = new Date(time < 0 ? 0 : time)
//           .toISOString()
//           .substr(11, 8);
//         return {
//           content: `<div class="timer-msg"><div class="timer-flavor">${flavor}</div><div class="timer-time">${frmtTime}</div><div class="timer-bar"><div></div></div><div class="complete-msg">${i18n(
//             "MonksChatTimer.Complete"
//           )}</div></div>`,
//         };
//       },
...
//           chatdata.speaker = ChatMessage._getSpeakerFromUser({
//             user: game.user,
//           });
//           chatdata.flags = {
//             core: { canPopout: true },
...
//             .toISOString()
//             .substr(11, 8);
//           return `<div class="timer-msg"><div class="timer-flavor">${flavor}</div><div class="timer-time">${frmtTime}</div><div class="timer-bar"><div></div></div><div class="complete-msg">${i18n(
//             "MonksChatTimer.Complete"
//           )}</div></div>`;
//         },
//         shouldDisplayToChat: true,
...
//         flags: { "monks-chat-timer": { complete: true } },
//       });
...
//         options.whisper = ChatMessage.getWhisperRecipients("GM").map(
//           (u) => u.id
//         );
...
//         let tokenOwners = tkn
//           ? Object.entries(tkn?.actor.ownership)
//               .filter(([k, v]) => {
//                 return v == CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER;
//               })
//               .map((a) => {
//                 return a[0];
//               })
//           : [];
//         options.whisper = Array.from(
//           new Set(
...
//       return `<span class="logic-style">${
//         trigger.name
//       }</span> count <span class="details-style">"${
//         action.data.duration
//       }"</span> for <span class="value-style">&lt;${i18n(
//         trigger.values.for[action.data?.for]
//       )}&gt;</span>`;
//     },
//   });
// });
