...
const NarratorTools = {
  ...
  _chatMessage(message, content, chatData) {
    ...
    if (game.user.role >= game.settings.get(MODULE, "PERMDescribe")) {
      commands.description = new RegExp(
        "^\\/desc(?:ribe|ription|) ([^]*)",
        "i"
      );
      commands.notification = new RegExp(
        "^\\/not(?:e|ify|ication) ([^]*)",
        "i"
      );
    }
    ...
    if (
      game.user.role >= game.settings.get(MODULE, "PERMAs") &&
      this.character &&
      !/^\/.*/.exec(content)
    ) {
      ChatMessage.create({
    ...
    if (narration.display) {
      const scroll = () => {
        if (!this.sharedState.narration.paused) {
          ...
          let duration = this.messageDuration(
            this.sharedState.narration.message.length
          );
          if (scroll > 20) {
            const remaining =
              1 -
              Number(this.elements.content[0].style.top.slice(0, -2)) / -scroll;
            const duration_multiplier = game.settings.get(
              MODULE,
              "DurationMultiplier"
            );
            const scroll_duration =
              (duration - 500 - 4500 * duration_multiplier) * remaining;
            const fun_scroll = () => {
      ...
      if (narration.id !== this._id) {
        ...
        this.elements.buttonCopy[0].style.display = game.settings.get(
          MODULE,
          "Copy"
        )
          ? ""
        ...
        this._timeouts.narrationOpens = +setTimeout(() => {
          ...
          this.elements.buttons[0].style.top = `calc(50% + ${
            60 + height / 2
          }px)`;
          this._updateStopButton(
            game.settings.get(MODULE, "NarrationStartPaused")
          );
  ...
  _loadFont(font) {
    ...
    style.appendChild(
      document.createTextNode(
        `@font-face {font-family: NTCustomFont; src: url('${font}');}`
      )
    );
  ...
  _pause() {
    const canScenery =
      game.user.role >= game.settings.get(MODULE, "PERMScenery");
  ...
  _ready() {
    ...
    this.isNarrator =
      ...
      game.user.role >= game.settings.get(MODULE, "PERMNarrate");
    ...
    this.elements.buttonClose.html(
      `<i class="fas fa-times-circle"></i> ${game.i18n.localize("Close")}`
    );
    this.elements.buttonClose.on("click", this._narrationClose);
    this.elements.buttonCopy.html(
      `<i class="fas fa-clipboard"></i> ${game.i18n.localize("NT.Copy")}`
    );
    ...
    document.addEventListener("contextmenu", (ev) => {
      if (
        ev.target.classList.contains("journal-entry-pages") ||
  ...
  _registerGameSettings() {
    ...
    game.settings.register(MODULE, "BGImage", {
      ...
      scope: "world",
      config: true,
      default: "systems/pf2e/icons/iconics/Amiri.webp",
      type: String,
  ...
  _fitSidebar() {
    const sidebarWidth = $("body").find("div#sidebar.app.collapsed").length
      ? 0
      : 305;
  ...
  _updateStopButton(pause) {
    if (pause) {
      NarratorTools.elements.buttonPause.html(
        `<i class='fas fa-play-circle'></i> ${game.i18n.localize(
          "NT.PlayButton"
        )}`
      );
    } else {
      NarratorTools.elements.buttonPause.html(
        `<i class='fas fa-pause-circle'></i> ${game.i18n.localize(
          "NT.PauseButton"
        )}`
      );
    }
  ...
  _updateContentStyle() {
    ...
    this.elements.content[0].style.fontFamily = `${game.settings.get(
      MODULE,
      "WebFont"
    )}`
      ? "NTCustomFont"
      : "";
    this.elements.content[0].style.fontSize = `${game.settings.get(
      MODULE,
      "FontSize"
    )}`;
    this.elements.content[0].style.color = `${game.settings.get(
      MODULE,
      "TextColor"
    )}`;
    this.elements.content[0].style.textShadow = `${game.settings.get(
      MODULE,
      "TextShadow"
    )}`;
  },
  ...
  messageDuration(length) {
    return (
      (Math.clamp(2000, length * 80, 20000) + 3000) *
        game.settings.get(MODULE, "DurationMultiplier") +
      500
    );
  },
  scenery(state) {
  ...
  sharedState: {
    ...
    set narration(state) {
      const sharedState = {
        ...game.settings.get(MODULE, "sharedState"),
        narration: state,
      };
...
Hooks.on(
  "renderChatMessage",
  NarratorTools._renderChatMessage.bind(NarratorTools)
);
Hooks.on(
  "renderSceneControls",
  NarratorTools._createSceneryButton.bind(NarratorTools)
);
